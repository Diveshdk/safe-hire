...
import { performOCR, extractAadhaarDetails } from "@/lib/ocr"
...
export async function verifyOfflineXml(
  ...
  try {
    ...
    // xml-crypto v6+: pass publicCert via constructor options
    const signedXml = new SignedXml({ publicCert: Buffer.from(certPem!) })
...
export async function verifyAadhaarOCR(
  imageBuffer: Buffer,
  fileName: string,
): Promise<{ success: boolean; fullName?: string; aadhaarNumber?: string; message?: string }> {
  try {
    const text = await performOCR(imageBuffer, fileName)
    if (!text) {
      return { success: false, message: "OCR could not extract any text from the image." }
    }

    const { fullName, aadhaarNumber } = extractAadhaarDetails(text)

    if (!aadhaarNumber) {
      console.log("[OCR Debug] Raw Text (No Aadhaar found):\n", text)
      return { success: false, message: "Could not find a valid 12-digit Aadhaar number in the image." }
    }

    if (!fullName) {
      console.log("[OCR Debug] Raw Text (No Name found):\n", text)
      return {
        success: false,
        aadhaarNumber,
        message: "Found Aadhaar number but could not extract name accurately. Please ensure the card is clearly visible.",
      }
    }

    return { success: true, fullName, aadhaarNumber }
  } catch (err: any) {
    return { success: false, message: err?.message || "OCR processing failed" }
  }
}
